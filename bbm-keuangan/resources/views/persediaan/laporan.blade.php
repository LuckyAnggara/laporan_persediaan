    ...
    <div class="container-fluid container-lg mx-auto mt-5">
        ...
        <form action="{{ route('laporan') }}" method="get">
            <div class="row my-2 d-print-none">
                ...
                <div class="col-3">
                    <select class="form-select form-select-lg" name="tanggal">
                        @foreach($tanggalData as $key=> $t)
                        <option value="{{$t->date}}" {{$tanggal == $t->date ? 'selected' : ''}}>{{date('d F Y', strtotime($t->date))}}</option>
                        @endforeach
